...
sed -i -e '1i \
track type=interact name="interact "$LABEL"_"$RESOLUTION"" description=""$LABEL" loops at "$RESOLUTION"" interactDirectional=true maxHeightPixels=200:100:50 visibility=full\\nbrowser position "$REGION"' "$LABEL"_"$RESOLUTION".interact"> write_interact.sh
